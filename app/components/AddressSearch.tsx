...
export default function AddressSearch() {
  ...
  async function selectAddress(suggestion: AddressSuggestion) {
    ...
    setSelectedBy(suggestion.adresse.postnrnavn);
  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      ...
      {/* Resultat */}
      {result && (
        <div className={`relative z-10 mt-8 rounded-2xl p-8 border-2 ${riskConfig[result.riskLevel].bg} ${riskConfig[result.riskLevel].border} shadow-xl`}>
          ...
          {/* Luftfoto — over risikoscoren */}
          <div className="mb-6">
            <Luftfoto
              adresse={result.address.split(",")[0]?.trim() ?? result.address}
              postnr={result.postnr}
              by={
                selectedBy ||
                result.address.split(",")[1]?.trim().replace(/^\d{4}\s*/, "") ||
                ""
              }
          ...
          {result.riskLevel !== "low" && (
            <div className="relative z-10 bg-white rounded-xl p-6 text-center">
              <p className="text-gray-600 mb-4">Vi anbefaler en professionel vurdering af din bolig</p>
              <Link
                href="/tilbud"
                className="inline-block bg-[#e67e22] hover:bg-[#f39c12] text-white font-bold py-3 px-8 rounded-xl transition shadow-lg cursor-pointer"
              >
