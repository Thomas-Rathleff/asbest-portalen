      ...
      <section className="bg-gradient-to-br from-[#1a365d] to-[#2a4a7f] text-white py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-start gap-6">
            <div className="w-20 h-20 bg-[#e67e22] rounded-2xl flex items-center justify-center text-3xl font-bold flex-shrink-0">A</div>
            <div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 grid md:grid-cols-3 gap-8">
          <div className="md:col-span-2 space-y-6">
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
              ...
              <p className="text-gray-500 leading-relaxed">Denne virksomhed er autoriseret af Sikkerhedsstyrelsen til at udføre asbestfjernelse og miljøsanering. Alle opgaver udføres i overensstemmelse med BEK 744 og gældende arbejdsmiljølovgivning.</p>
            </div>
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
              ...
              <ul className="space-y-3">
                {["Asbestfjernelse og -sanering","Nedtagning af asbesttag (eternit)","Anmeldelse til Arbejdstilsynet","Korrekt bortskaffelse af asbestaffald","Miljøsanering"].map(y => (
                  <li key={y} className="flex items-center gap-3 text-gray-700">
                    <div className="w-5 h-5 rounded-full bg-[#27ae60] flex-shrink-0 flex items-center justify-center"><svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7"/></svg></div>
                    {y}
                  </li>
                ))}
              </ul>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
              ...
              <div className="space-y-3 text-sm text-gray-600">
                <div><div className="font-semibold text-gray-800">Adresse</div><div>{v.adresse}</div><div>{v.postnr} {v.by}</div></div>
                {v.cvr && <div><div className="font-semibold text-gray-800">CVR</div><a href={`https://www.cvr.dk/virksomhed/${v.cvr}`} target="_blank" rel="noopener noreferrer" className="text-[#e67e22] hover:underline">{v.cvr} →</a></div>}
                {v.asbe_nr && <div><div className="font-semibold text-gray-800">Autorisationsnr.</div><div className="font-mono">{v.asbe_nr}</div></div>}
              </div>
            </div>
            <div className="bg-[#1a365d] rounded-xl p-6 text-white text-center">
              <div className="text-lg font-bold mb-2">Få et tilbud</div>
              <p className="text-blue-200 text-sm mb-4">Gratis asbestvurdering af din bolig</p>
              <Link href="/" className="block bg-[#e67e22] hover:bg-[#f39c12] text-white font-bold py-3 px-4 rounded-xl transition text-sm">Tjek din bolig →</Link>
            </div>
          </div>
        </div>
      </section>
